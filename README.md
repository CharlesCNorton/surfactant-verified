...
`coq/surfactant.v` is the source of truth. It defines the indication logic
...
```
coq/      surfactant.v            formalization, proofs, extraction directive
models/   surfactant.pml          Promela model for SPIN
          surfactant.xml          timed automata model for UPPAAL
ocaml/    surfactant_cli.ml       command-line decision interface
          test_surfactant.ml      unit tests against the extraction
          fuzz_surfactant.ml      randomized property tests
tools/    validate.py             reference cases, trial profiles, SPIN driver
          server.py               HTTP decision service
          test_server.py          service tests
data/     literature_validation_cases.json    trial-derived case definitions
```

`ocaml/surfactant_decision.ml` and its interface are extraction output and are
not tracked. Binaries build into `bin/`. Both are regenerated by `make`.

## Build

```bash
make coq      # compile coq/surfactant.v and extract the decision module
make ocaml    # build the CLI, unit tests, and fuzzer into bin/
...
make serve    # start the HTTP decision service
```
...
```bash
spin -a models/surfactant.pml       # syntax check and generate verifier
gcc -o pan pan.c                    # compile verifier
./pan -a                            # verify all properties
./pan -a -N p_contra_blocks         # verify one property
spin -p models/surfactant.pml       # print execution trace
spin -t models/surfactant.pml       # guided simulation after verification
```

`make spin` drives preprocessing and verification, emitting
`models/surfactant_pp.pml`.
...
SPIN 6.4.9, run 2026-01-04 against `models/surfactant.pml`.
...
Open `models/surfactant.xml`, then use the Simulator tab to step through transitions and
...
`tools/validate.py literature` runs the extraction against trial-derived
profiles in `data/literature_validation_cases.json` and writes
`VALIDATION_RESULTS.json`. The
